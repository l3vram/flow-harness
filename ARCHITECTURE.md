...
| `@flow/qa` | Deterministic QA engine. Layer A: `runQA` verifies each acceptance criterion with its own explicit command (spawnSync, no shell), writes evidence, emits a report + error tickets. Layer B seam: `runWebQA` over a `WebDriver` (structured web criteria; a `FakeWebDriver` for offline tests, a real Playwright driver later). Independent, run-less, offline; lib + bin `flow-qa`; emits evidence/verdicts/tickets, not decisions. **Self-built.** | — | ✅ v0.18 / v0.21 |
