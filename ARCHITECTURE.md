...
| `@flow/executor` | Writes **and edits** (safe search/replace) code, then runs the verify command. The only writer of product code. All failure paths (provider / apply / parse) become a failed verification for the repair loop — none aborts the run. | llm, context | ✅ v0.6 / v0.27 |
| `@flow/orchestrator` | The conductor: CEO → executor → runtime, with a **repair loop** (retry a failed verify by feeding the error back), **risk gating** (high-risk → human review), lesson recording, and **QA-verified tasks** (a task with acceptance criteria is verified by `@flow/qa`; its report decides green/blocked and its tickets drive the repair loop; evidence is collected run-scoped under the run dir; in objective mode it derives QA criteria from the planner's acceptance and attaches them to the final task via `@flow/verify`; every run self-scores via `@flow/eval`, attached to the report). `flow-run <config.json>`. | core, ceo, executor, llm, context, memory, review, qa, verify, eval | ✅ v0.7 / v0.20 / v0.23 / v0.25 / v0.32 |
...
| `@flow/eval` | Objective evaluation engine: `evaluate` scores a run 0–100 across dimensions from its facts (deterministic, no LLM); `compare` diffs runs across versions. **Self-built.** | — | ✅ v0.30 |
